...
TMP_AVI = PREVIEW / "cw001_v15_tmp.avi"
OUT_VID = PREVIEW / "cw001_v15_demo.mp4"
OUT_HTML= PREVIEW / "cw001_v15_demo.html"
...
RED    = (17,  15, 228)
GREEN  = (12, 220,  48)
YELLOW = (0,  210, 255)   # BGR 黄 — 标注/引导层专用色
WHITE  = (255, 255, 255)
LINE_W = 2; NODE_R = 3
...
PH1_END  = 90    # +1000ms draw-on red
PH2_END  = 110   #  +667ms arrow fly-in
PH3_END  = 210   # +3333ms push+recolor
PH4_END  = 240   # +1000ms hold green
...
# fi=150..158: flash衰减, 稳定绿线
FLASH_PEAK  = PH3_END - 1   # fi=209 (shifted by PH0=60)
FLASH_RISE  = 15             # flash 从 fi=194 开始上升 (领先颜色7帧)
FLASH_FALL  = 9
COLOR_START_FI = FLASH_PEAK - 8   # fi=201
COLOR_END_FI   = FLASH_PEAK       # fi=209
...
def _pil_label(img_bgr, lines, org, color_bgr=YELLOW, padding=14):
    """
    PIL 渲染: 中文字符正常显示 + 半透明黑底 + 黄色边框标签。
    lines: [(text, font_size), ...]
    org: (x, y) 标注框左上角
    返回: guide_src (标注框底部中心, 供引线起点)
    """
    from PIL import Image, ImageDraw, ImageFont
    fonts = [ImageFont.truetype(FONT_CN, sz) for _, sz in lines]
    pil_tmp = Image.new("RGB", (1, 1))
    d_tmp   = ImageDraw.Draw(pil_tmp)
    w_max = 0; total_h = 0; line_hs = []
    for (text, _), fnt in zip(lines, fonts):
        bb = d_tmp.textbbox((0, 0), text, font=fnt)
        lw, lh = bb[2]-bb[0], bb[3]-bb[1]
        w_max = max(w_max, lw); line_hs.append(lh); total_h += lh + 6
    box_w = w_max + padding * 2
    box_h = total_h + padding * 2
    x0, y0 = int(org[0]), int(org[1])
    H, W   = img_bgr.shape[:2]
    if x0 + box_w > W: x0 = W - box_w
    if y0 + box_h > H: y0 = H - box_h
    x1, y1 = x0 + box_w, y0 + box_h
    pil_img = Image.fromarray(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)).convert("RGBA")
    bg = Image.new("RGBA", pil_img.size, (0, 0, 0, 0))
    ImageDraw.Draw(bg).rectangle([x0, y0, x1, y1], fill=(0, 0, 0, 150))
    pil_img = Image.alpha_composite(pil_img, bg).convert("RGB")
    draw = ImageDraw.Draw(pil_img)
    r, g, b = color_bgr[2], color_bgr[1], color_bgr[0]  # BGR→RGB
    draw.rectangle([x0, y0, x1, y1], outline=(r, g, b), width=2)
    cur_y = y0 + padding
    for (text, _), fnt, lh in zip(lines, fonts, line_hs):
        draw.text((x0 + padding, cur_y), text, font=fnt, fill=(r, g, b))
        cur_y += lh + 6
    img_bgr[:] = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
    return (x0 + box_w // 2, y1)   # 引线起点

def draw_guide_arrow(img, src, tgt, color=YELLOW, pulse=1.0):
    """
    CUE_DESIGN_LANGUAGE §引导箭头 v2 (加粗 2x):
    引线 2px + 三角头 hw=8px, 黄色, 不可与 barb 混用。
    """
    av = np.array(tgt, dtype=float) - np.array(src, dtype=float)
    dist = np.linalg.norm(av)
    if dist < 1: return
    an  = av / dist
    tip = np.array(tgt, dtype=float) - an * 14
    c   = tuple(int(x * (0.5 + 0.5 * pulse)) for x in color)
    cv2.line(img, ip(src), ip(tip), c, 2, cv2.LINE_AA)
    perp = np.array([-an[1], an[0]])
    notch = tip - an * 14
    pts = np.array([ip(tip), ip(notch + perp*8), ip(notch - perp*8)], dtype=np.int32)
...
def draw_indicator_animated(img, fi):
    ...
    # ── Ph0: 标注层 v3 (PIL中文+边框+加粗引线) ──────────────────────────────
    if fi < PH0_END:
        alpha = min(1.0, fi / 10.0)
        pulse = 0.5 + 0.5 * math.sin(fi * math.pi / 8.0)
        ELBOW_PT = (417, 615)   # fr149 左肘实测
        LABEL_ORG = (420, 260)  # 右上方空白区

        if alpha > 0.02:
            ov = img.copy()
            guide_src = _pil_label(ov,
                [("鸡翅膀", 48), ("Chicken Wing", 36)],
                LABEL_ORG, YELLOW)
            draw_guide_arrow(ov, guide_src, ELBOW_PT, YELLOW, pulse)
