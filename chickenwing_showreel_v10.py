#!/usr/bin/env python3
"""
CUE-CHICKENWING-001 展示环 v10
闪光对齐: 峰值 = Ph3末帧(fi=149, t_push=1.0, 颜色刚完全变绿)
  flash 在 Ph3 最后8帧上升, 峰值在 fi=149, 之后8帧(Ph4开头)下降
  = "红→绿变色" 和 "闪光爆发" 完全同步
"""
...
TMP_AVI = PREVIEW / "cw001_v10_tmp.avi"
OUT_VID = PREVIEW / "cw001_v10_demo.mp4"
OUT_HTML= PREVIEW / "cw001_v10_demo.html"
...
PH1_END  = 30    # 1000ms draw-on red
PH2_END  = 50    #  667ms arrow fly-in
PH3_END  = 150   # 3333ms push+recolor
PH4_END  = 180   # 1000ms hold green

COLOR_LEAD = 0.60   # color change starts when push is 60% done

# ── flash window: centered on fi=PH3_END-1 = 149 ─────────────────────────────
FLASH_PEAK  = PH3_END - 1   # fi=149, t_push=1.0, the "变色完成" moment
FLASH_RISE  = 10             # frames before peak (rise, within Ph3)
FLASH_FALL  = 9              # frames after peak (fall, within Ph4)
# full flash window: fi 139..158
...
print(f"  Rise: {FLASH_RISE} fr in Ph3 final; Fall: {FLASH_FALL} fr in Ph4 start")
...
def draw_indicator_animated(img, fi):
    ...
    # ── Ph3: push + early recolor (+ flash rises at end) ─────────────────────
    elif fi < PH3_END:
        t_push = ss((fi-PH2_END)/(PH3_END-PH2_END-1))
        curr_le = le_ + t_push*(ge_-le_)

        # color starts at COLOR_LEAD, completes at t_push=1.0
...
<title>SwingCue · Chicken Wing · fo-wrong-4 · v10</title>
<style>
...
<div class="title">
  ...
  <span>CHICKEN WING v10</span>
</div>
<div class="stage">
  <video id="vid" src="cw001_v10_demo.mp4" preload="auto"></video>
