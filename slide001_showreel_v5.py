...
TMP_AVI = PREVIEW / "slide001_v5_tmp.avi"
OUT_VID = PREVIEW / "slide001_v5_demo.mp4"
...
def _pil_label(img_bgr, lines, org, color_bgr=YELLOW, padding=14):
    """PIL渲染: 中文正常 + 半透明黑底 + 边框标签。返回引线起点(标注底中心)。"""
    from PIL import Image, ImageDraw, ImageFont
    fonts = [ImageFont.truetype(FONT_CN, sz) for _, sz in lines]
    pil_tmp = Image.new("RGB", (1, 1)); d_tmp = ImageDraw.Draw(pil_tmp)
    ...
    box_w = w_max + padding*2; box_h = total_h + padding*2
    ...
    for (text,_), fnt, lh in zip(lines, fonts, line_hs):
        draw.text((x0+padding, cur_y), text, font=fnt, fill=(r,g,b)); cur_y += lh+6
    img_bgr[:] = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
    return (x0 + box_w//2, y1)

def draw_guide_arrow(img, src, tgt, color=YELLOW, pulse=1.0):
    """CUE_DESIGN_LANGUAGE §引导箭头 v2 (加粗2x): 引线2px + 三角头hw=8px。"""
...
def draw_freeze(img, fi):
    ...
    if fi < PH0_END:
        ...
        LABEL_ORG  = (430, 270)   # 右上方空白区, 同侧

        if alpha > 0.02:
            ov = img.copy()
            guide_src = _pil_label(ov,
                [("髋部横滑", 48), ("Hip Slide", 36)],
                LABEL_ORG, YELLOW)
