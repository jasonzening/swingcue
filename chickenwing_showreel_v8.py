#!/usr/bin/env python3
"""
CUE-CHICKENWING-001 展示环 v8 (= v8+v9 合并)
v8: 变色时机提前 — 推到 60% 时就开始变绿 (而非到位后才变)
v9: 到位瞬间闪光特效 — 8帧 267ms 辉光爆闪 + 节点脉冲
"""
...
TMP_AVI = PREVIEW / "cw001_v8_tmp.avi"
OUT_VID = PREVIEW / "cw001_v8_demo.mp4"
OUT_HTML= PREVIEW / "cw001_v8_demo.html"
...
# ── Phase boundaries (same total = 6000ms, same as v7) ───────────────────────
FREEZE_NFRAM = 180

PH1_END = 30    # 0..29    1000ms  draw-on red
PH2_END = 50    # 30..49    667ms  arrow fly-in
PH3_END = 150   # 50..149  3333ms  push + early recolor (combined, no split)
PH4_END = 180   # 150..179 1000ms  hold green (first 8fr = flash)

FLASH_FR = 8    # 267ms flash burst at Ph4 start

# v8 key: color starts when push is COLOR_LEAD% done (not at 100%)
COLOR_LEAD = 0.60   # color change begins at 60% travel
...
def draw_barb(img, tip_np, fwd_np, color, scale=1.0):
    perp     = np.array([-fwd_np[1], fwd_np[0]])
    al, ahw  = ARROW_LEN*scale, ARROW_HW*scale
    tail_c   = tip_np - fwd_np*al
    notch_pt = tip_np - fwd_np*(al*(1-ARROW_NOTCH))
    pts = np.array([ip(tip_np),
                    ip(tail_c+perp*ahw),
                    ip(notch_pt),
                    ip(tail_c-perp*ahw)], dtype=np.int32)
...
print(f"  ge={ge_.astype(int)} le→ge={np.linalg.norm(ge_-le_):.1f}px  COLOR_LEAD={COLOR_LEAD}")

# ── flash intensity curve (frames 0..FLASH_FR-1) ─────────────────────────────
# ramp up frames 0-2, peak at 2, decay 3-7
FLASH_CURVE = [0.5, 0.85, 1.0, 0.85, 0.65, 0.45, 0.25, 0.10]   # 8 values
...
def draw_indicator_animated(img, fi):
    ...
    # ── Ph2: full red arm + arrow flies in ───────────────────────────────────
        ...
        t_ph = ss((fi-PH2_END+PH2_END-PH1_END-(PH2_END-PH1_END))/(PH2_END-PH1_END-1))
        # simpler: normalized local
        t_ph = ss((fi-PH1_END)/(PH2_END-PH1_END-1))
        tip = (le_+outN_*STANDOFF)+t_ph*(le_-(le_+outN_*STANDOFF))
        draw_barb(img, tip, push_dir, RED)

    # ── Ph3: push + early recolor ────────────────────────────────────────────
    elif fi < PH3_END:
        t_push = ss((fi-PH2_END)/(PH3_END-PH2_END-1))
        curr_le = le_+t_push*(ge_-le_)

        # color: starts changing when t_push >= COLOR_LEAD
        if t_push < COLOR_LEAD:
            t_col = 0.0
        else:
            t_col = ss((t_push - COLOR_LEAD) / (1.0 - COLOR_LEAD))

        col = lerp_color(RED, GREEN, t_col)

        # glow: fades in as t_col grows
        if t_col > 0.05:
        ...
        # correct-elbow dot appears when t_col > 0.3
        if t_col > 0.3:
        ...
        # arrow: color tracks t_col (early change), standoff shrinks
        cur_so = STANDOFF*(1-t_push)+2
        tip = curr_le+outN_*cur_so
        draw_barb(img, tip, push_dir, col)

    # ── Ph4: hold green + flash burst at start ───────────────────────────────
    else:
        fi_local = fi - PH3_END   # 0..29

        # base: stable green line
        ov = img.copy()
        cv2.line(ov, ip(ls_), ip(lw_), GREEN, 9, cv2.LINE_AA)
        ...
        # flash burst (first FLASH_FR frames)
        if fi_local < FLASH_FR:
            fint = FLASH_CURVE[fi_local]   # 0..1

            # ① wide glow explosion
            ov2 = img.copy()
            for gw, ga in [(35, 0.55*fint), (22, 0.40*fint), (12, 0.30*fint)]:
                cv2.line(ov2, ip(ls_), ip(lw_), GREEN, gw, cv2.LINE_AA)
            cv2.addWeighted(ov2, ga, img, 1-ga, 0, img)

            # ② bright core — slightly white-tinted at peak
            flash_col = lerp_color(GREEN, WHITE, 0.45*fint)
            cv2.line(img, ip(ls_), ip(lw_), flash_col, LINE_W+1, cv2.LINE_AA)

            # ③ elbow node pulse (expands then shrinks)
            pulse_r = int(NODE_R + 7*fint)
            pulse_col = lerp_color(GREEN, WHITE, 0.5*fint)
            cv2.circle(img, ip(ge_), pulse_r, pulse_col, -1, cv2.LINE_AA)
...
if r.returncode != 0: print(r.stderr[-400:]); raise RuntimeError
...
<title>SwingCue · Chicken Wing · fo-wrong-4 · v8</title>
<style>
...
<div class="title">
  ...
  <span>CHICKEN WING v8</span>
</div>
<div class="stage">
  <video id="vid" src="cw001_v8_demo.mp4" preload="auto"></video>
...
<div class="legend">
  ...
  <div class="litem" style="border-color:#2a3a1a;color:#574;flex:2">③ 推移+变色<br>3333ms ★</div>
  <div class="litem" style="border-color:#1a4a1a;color:#4a4">④ ⚡绿线<br>1000ms</div>
</div>
...
vid.addEventListener('timeupdate',()=>{{
  ...
  if(inCue){{
    ...
    const ph=pct<0.167?'① 画红线':pct<0.278?'② 箭头出现':pct<0.833?'③ 推移→变色':'④ ⚡ 对了!';
...
print(f"""
变色设计 (v8):
  COLOR_LEAD=0.60 — 推到60%时开始变色 (而非v7的100%)
  t_push=0.60: 肘在 le+60%*(ge-le), 颜色开始红→绿
  t_push=1.00: 肘到达ge, 颜色完全绿 → 无滞后

闪光特效 (v9):
  Ph4 前 8帧 (267ms): glow爆闪(35px宽) + 白色节点脉冲
  FLASH_CURVE={FLASH_CURVE}
  峰值帧2: 宽35px glow (0.55α) + 白色核心线 + 节点r=10
""")
