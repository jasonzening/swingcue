"""
render_compare_v4.py — PR-8b.1 visual verification with head_crown.
...
Outputs to docs/PR-8b_VISUAL_VERIFY/v4_z_stabilized/:
...
OUT_DIR = ROOT / "docs/PR-8b_VISUAL_VERIFY/v4_z_stabilized"
...
def main() -> int:
    print(f"[render_compare_v4] video      = {VIDEO}")
    print(f"[render_compare_v4] smoke_json = {SMOKE_JSON}")
    print(f"[render_compare_v4] out_dir    = {OUT_DIR}")

    # WHAM from new smoke JSON.
    wham_joints_by_fi, wham_crown_by_fi = load_wham_2d(SMOKE_JSON)
    print(f"[render_compare_v4] WHAM frames: {len(wham_joints_by_fi)} "
          f"(crown valid: {sum(1 for c in wham_crown_by_fi.values() if c is not None)})")

    # MediaPipe live.
    print(f"[render_compare_v4] running MediaPipe ...")
    mp_2d_by_idx = run_mediapipe(VIDEO)
    print(f"[render_compare_v4] MediaPipe frames: {len(mp_2d_by_idx)}")

    # Coord log
    log_lines = [
        f"# render_compare_v4 — head_crown coord dump for KEY_FRAMES={KEY_FRAMES}",
    ...
    for fi in KEY_FRAMES:
        ...
        print(f"[render_compare_v4] wrote {sbs_path.name}  {sbs.shape[1]}x{sbs.shape[0]}")
    cap.release()
    ...
    if ok:
        ...
        print(f"[render_compare_v4] wrote {torso_path.name}  {torso.shape[1]}x{torso.shape[0]}")
    ...
    cap.release()
    writer.release()
    print(f"[render_compare_v4] wrote overlay_full.mp4 ({fi} frames, {mp4_path.stat().st_size//1024} KB)")
    print(f"[render_compare_v4] DONE — {OUT_DIR}")
    return 0
