#!/usr/bin/env python3
"""
CUE-CHICKENWING-001 展示环 v5
定格内动画序列:
  Phase 1: 红线从上往下 draw-on
  Phase 2: 箭头从肘外飞入
  Phase 3: 箭头推动 — 红线形变(肘移)→ 变绿 (推直)
  Phase 4: 定格绿线 hold
然后 time remapping 继续播 (结构与 v4 完全相同, 只换 draw_indicator_animated)
"""
...
TMP_AVI = PREVIEW / "cw001_v5_tmp.avi"
OUT_VID = PREVIEW / "cw001_v5_demo.mp4"
OUT_HTML= PREVIEW / "cw001_v5_demo.html"
...
FPS_OUT     = 30
FR_FREEZE   = 149
FREEZE_SEC  = 3.0                         # 3s so sequence has room
FREEZE_NFRAM= int(FREEZE_SEC * FPS_OUT)   # 90 frames
EASE_NFRAM  = 25
NORMAL_RATE = 0.25
...
# ── Phase boundaries (in freeze-local frame index) ───────────────────────────
PH1_END = 18   # 0..17  draw-on red arm
PH2_END = 30   # 18..29 arrow flies in
PH3_END = 72   # 30..71 push + morph  (42 frames)
PH4_END = FREEZE_NFRAM  # 72..89 hold green (18 frames)

# ── helpers ────────────────────────────────────────────────────────────────────
def ss(t): return t*t*(3-2*t)          # smoothstep
def lerpf(a,b,t): return a + t*(b-a)
def lerp_color(c1,c2,t):
    return tuple(int(c1[i]+t*(c2[i]-c1[i])) for i in range(3))
def ip(a): return (int(round(float(a[0]))), int(round(float(a[1]))))

def draw_barb(img, tip_np, fwd_np, color=GREEN):
    """tip = numpy 2d; fwd = normalized direction (tail→tip); color BGR."""
...
kp_freeze = get_kp_np(FR_FREEZE)
ls_ = kp_freeze['left_shoulder']; le_ = kp_freeze['left_elbow']
lw_ = kp_freeze['left_wrist']
sw_ = lw_ - ls_
t_  = float(np.clip(np.dot(le_-ls_, sw_)/(np.dot(sw_,sw_)+1e-9), 0.1, 0.9))
ge_ = ls_ + t_*sw_
out_= le_ - ge_; outN_ = out_/(np.linalg.norm(out_)+1e-9)
seg1_len = np.linalg.norm(le_-ls_)
seg2_len = np.linalg.norm(lw_-le_)
total_arm = seg1_len + seg2_len

arrow_fly_start = le_ + outN_*STANDOFF   # where arrow comes from (outside)
arrow_fly_end   = le_                     # arrow lands at elbow
# push direction: inward = -outN_
push_dir = -outN_

print(f"fr149 geometry: ls={ls_.astype(int)} le={le_.astype(int)} lw={lw_.astype(int)}")
print(f"  ge={ge_.astype(int)} outN={outN_.round(3)}")
print(f"  arm total={total_arm:.1f}px  le->ge={np.linalg.norm(ge_-le_):.1f}px")

# ── per-freeze-frame indicator drawing ────────────────────────────────────────
def draw_indicator_animated(img, fi):
    """fi: local freeze frame index 0..FREEZE_NFRAM-1"""

    # ── Phase 1: draw-on red arm (top → bottom) ─────────────────────────────
    if fi < PH1_END:
        t_ph = ss(fi / (PH1_END-1)) if fi > 0 else 0.0
        draw_len = t_ph * total_arm

        if draw_len <= seg1_len:
            end_pt = ls_ + (le_-ls_) * (draw_len / seg1_len)
            cv2.line(img, ip(ls_), ip(end_pt), RED, LINE_W, cv2.LINE_AA)
        else:
            cv2.line(img, ip(ls_), ip(le_), RED, LINE_W, cv2.LINE_AA)
            rem = draw_len - seg1_len
            end_pt = le_ + (lw_-le_) * (rem/seg2_len)
    ...
    # ── Phase 2: red arm fully shown + arrow flies in ────────────────────────
    elif fi < PH2_END:
        # full red arm
        cv2.line(img, ip(ls_), ip(le_), RED, LINE_W, cv2.LINE_AA)
        ...
        # arrow: fly from arrow_fly_start → le_
        t_ph = ss((fi - PH1_END) / (PH2_END - PH1_END - 1))
        tip  = arrow_fly_start + t_ph*(arrow_fly_end - arrow_fly_start)
        draw_barb(img, tip, push_dir, RED)

    # ── Phase 3: push + morph ─────────────────────────────────────────────────
    elif fi < PH3_END:
        t_ph = ss((fi - PH2_END) / (PH3_END - PH2_END - 1))

        # elbow moves le_ → ge_ (being pushed inward)
        curr_le = le_ + t_ph*(ge_ - le_)

        # color interpolates RED → GREEN
        col = lerp_color(RED, GREEN, t_ph)

        # glow (grows in with morph)
        ov = img.copy()
        cv2.line(ov, ip(ls_), ip(lw_), GREEN, 9, cv2.LINE_AA)
        cv2.addWeighted(ov, 0.18*t_ph, img, 1-0.18*t_ph, 0, img)
        ...
        # green correct-elbow dot fades in after halfway
        if t_ph > 0.4:
            ge_a = min(1.0, (t_ph-0.4)/0.3)
            ov2 = img.copy()
            cv2.circle(ov2, ip(ge_), max(1,int(NODE_R*0.83)), GREEN, -1, cv2.LINE_AA)
            cv2.addWeighted(ov2, ge_a, img, 1-ge_a, 0, img)

        # arrow tip stays just outside curr_le (tracks the elbow being pushed)
        # standoff shrinks from STANDOFF→0 as elbow reaches ge
        cur_standoff = STANDOFF * (1-t_ph) + 2   # 2px minimum clearance
        tip = curr_le + outN_*cur_standoff
...
# ── build timeline (same logic as v4) ────────────────────────────────────────
ease_advance = sum(NORMAL_RATE*(1-ss(i/EASE_NFRAM)) for i in range(EASE_NFRAM))
...
for idx, (sf, fi_local) in enumerate(timeline):
    if idx % 200 == 0:
        print(f"  {idx}/{n_total}  src={sf}  fi_local={fi_local}", flush=True)
...
<title>SwingCue · Chicken Wing · fo-wrong-4 · v5</title>
<style>
...
<div class="title">
  ...
  <span>CHICKEN WING v5</span>
</div>
<div class="stage">
  <video id="vid" src="cw001_v5_demo.mp4" preload="auto"></video>
...
<div class="legend">
  <div class="litem" style="border-color:#3a1a1a;color:#884">① 红线</div>
  <div class="litem" style="border-color:#2a2a1a;color:#884">② 箭头出现</div>
  <div class="litem" style="border-color:#1a3a1a;color:#484">③ 推直→绿</div>
  <div class="litem" style="border-color:#1a3a1a;color:#484">④ 绿线停</div>
</div>
<div class="hint">SPACE play/pause  ·  drag red zone to replay cue  ·  ← → 0.1s step</div>
<script>
...
vid.addEventListener('timeupdate',()=>{{
  ...
  if(inCue){{
    const pct=Math.round((t-FS)/(FE-FS)*100);
    const ph=pct<20?'① 画红线':pct<33?'② 箭头出现':pct<80?'③ 推直':' ④ 绿线';
    cueD.textContent='⚠ CHICKEN WING  '+ph;
    cueD.style.color='rgb(228,80,80)';
  }} else {{
    cueD.textContent='──'; cueD.style.color='#444';
  }}
}});
...
print(f"=> {OUT_HTML}")
print(f"\nPhase breakdown within freeze ({FREEZE_NFRAM} frames = {FREEZE_SEC}s):")
print(f"  Ph1 draw-on red:    fr 0..{PH1_END-1}   ({PH1_END} fr = {PH1_END/FPS_OUT:.2f}s)")
print(f"  Ph2 arrow fly-in:   fr {PH1_END}..{PH2_END-1}  ({PH2_END-PH1_END} fr = {(PH2_END-PH1_END)/FPS_OUT:.2f}s)")
print(f"  Ph3 push+morph:     fr {PH2_END}..{PH3_END-1}  ({PH3_END-PH2_END} fr = {(PH3_END-PH2_END)/FPS_OUT:.2f}s)")
print(f"  Ph4 hold green:     fr {PH3_END}..{PH4_END-1}  ({PH4_END-PH3_END} fr = {(PH4_END-PH3_END)/FPS_OUT:.2f}s)")
