...
TMP_AVI = PREVIEW / "slide001_v3_tmp.avi"
OUT_VID = PREVIEW / "slide001_v3_demo.mp4"
...
def draw_freeze(img, fi):
    ...
    if fi < PH0_END:
        ...
        # 目标: 髋外缘点 (impact fr147 实测)
        hip_target = np.array([float(OX_F), float(HIP_Y)])
        ...
        # 短箭头: 文字左下 → 髋外缘
        txt_anchor = np.array([float(tx), float(ty + 44)])
        av   = hip_target - txt_anchor
        an   = av / (np.linalg.norm(av) + 1e-9)
        tip  = hip_target - an * 28
