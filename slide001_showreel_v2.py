...
TMP_AVI = PREVIEW / "slide001_v2_tmp.avi"
OUT_VID = PREVIEW / "slide001_v2_demo.mp4"
...
FREEZE_NFRAM = 195   # 6500ms: Ph0=15 + Ph1~Ph4=180
PH0_END = 15     #  500ms 名称+箭头
PH1_END = 45     # +1000ms 红爆闪
PH2_END = 65     #  +667ms 箭头飞入
PH3_END = 165    # +3333ms 推移变绿
PH4_END = 195    # +1000ms hold

RFLASH_PEAK = 20; RFLASH_RISE = 5; RFLASH_FALL = 8    # shifted +15
...
def draw_freeze(img, fi):
    ...
    if fi < PH0_END:
        ...
        alpha = min(1.0, t_ph * 3.0)
        if alpha > 0.05:
            ov = img.copy()
            cv2.putText(ov, "\u9ac0\u90e8\u6a2a\u6ed1",   (30, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.4, RED, 3, cv2.LINE_AA)
            cv2.putText(ov, "Hip Slide", (30, 135),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.85, RED, 2, cv2.LINE_AA)
            # 指向箭头: 从文字右侧 → 髋部外缘红线位置
            txt_end = np.array([250.0, 112.0])
            tgt     = np.array([float(OX_F), float(HIP_Y)])
            av = tgt - txt_end; an = av/(np.linalg.norm(av)+1e-9)
            tip = tgt - an * 30
            draw_barb(ov, tip, an, RED)
            cv2.line(ov, ip(txt_end), ip(tip - an*22), RED, LINE_W, cv2.LINE_AA)
