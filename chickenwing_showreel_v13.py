...
TMP_AVI = PREVIEW / "cw001_v13_tmp.avi"
OUT_VID = PREVIEW / "cw001_v13_demo.mp4"
OUT_HTML= PREVIEW / "cw001_v13_demo.html"
...
def draw_indicator_animated(img, fi):
    ...
    # ── Ph0: 就近标注 + 闪动箭头 (CUE_DESIGN_LANGUAGE §Ph0 v2) ─────────────
    if fi < PH0_END:
        alpha = min(1.0, fi / 10.0)          # 前10帧淡入，之后保持
        pulse = 0.5 + 0.5 * math.sin(fi * math.pi / 8.0)   # 8帧周期脉冲

        # 展示帧左肘坐标 (fr149 实测)
        elbow_pt = np.array([417.0, 615.0])

        # 文字锚: 肘右侧同侧，就近
        tx = int(elbow_pt[0]) + 28
        ty = int(elbow_pt[1]) - 65

        # 短箭头: 从文字左下角 → 肘
        txt_anchor = np.array([float(tx), float(ty + 40)])
        av   = elbow_pt - txt_anchor
        an   = av / (np.linalg.norm(av) + 1e-9)
        tip  = elbow_pt - an * 28

        if alpha > 0.02:
            ov = img.copy()
            cv2.putText(ov, "\u9e21\u7fc5\u8180", (tx, ty),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, RED, 3, cv2.LINE_AA)
            cv2.putText(ov, "Chicken Wing", (tx, ty + 52),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, RED, 2, cv2.LINE_AA)
            cv2.line(ov, ip(txt_anchor), ip(tip - an * 22), RED, 2, cv2.LINE_AA)
            barb_col = tuple(int(c * (0.7 + 0.3 * pulse)) for c in RED)
            draw_barb(ov, tip, an, barb_col)
